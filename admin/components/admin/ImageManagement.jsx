...
const ImageManagement = () => {
  ...
  const loadImages = async () => {
    setIsLoading(true);
    try {
      const response = await fetchWithRefresh(`${apiUrl}/api/images`, {
        method: 'GET'
      });
  ...
  const handleDeleteImage = async (imageId) => {
    try {
      const response = await fetchWithRefresh(`${apiUrl}/api/images/${imageId}`, {
        method: 'DELETE'
      });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const deletePromises = selectedImages.map(id => 
        fetchWithRefresh(`${apiUrl}/api/images/${id}`, {
          method: 'DELETE'
        })
      );
