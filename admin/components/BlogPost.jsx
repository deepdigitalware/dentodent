...
export default function BlogPost({ slug }) {
  ...
  // Try to resolve a local cover image from assets by slug
  ...
  return (
    <article className="py-16">
      <Helmet>
        ...
        <meta property="og:image" content={localCoverUrl ? localCoverUrl : (isRemote ? `${post.cover}?w=1200&h=630&fit=crop` : post.cover)} />
        <script type="application/ld+json">{JSON.stringify({
          '@context': 'https://schema.org',
          '@type': 'BlogPosting',
          headline: post.title,
          image: localCoverUrl ? localCoverUrl : post.cover,
