...
import { Upload, Trash2, Eye, Download, Image as ImageIcon, Star, Copy, Check } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { toast } from 'react-hot-toast';
import { useContent } from '@/contexts/ContentContext';

const ImageManagement = () => {
  const { apiUrl, content, updateContent } = useContent();
  ...
  const copyImageUrl = async (url, id) => {
    try {
  ...
  const useImageInSection = async (section, imageUrl) => {
    try {
      const current = content[section] || {};
      let payload = { ...current };

      // Decide which field to set
      if (section === 'branding' || section === 'logos') {
        payload = { ...current, logoUrl: imageUrl };
      } else if (section === 'favicons') {
        payload = { ...current, faviconUrl: imageUrl };
      } else {
        payload = { ...current, imageUrl };
      }

      await updateContent(section, payload);
      toast.success('Linked image to content');
    } catch (e) {
              ...
              <div className="relative">
                ...
                <div className="absolute inset-0 bg-black bg-opacity-0 hover:bg-opacity-50 transition-all duration-200 flex items-center justify-center opacity-0 hover:opacity-100">
                  <div className="flex space-x-2">
                    ...
                    <Button
                      onClick={() => useImageInSection(activeCategory, image.url)}
                      className="bg-blue-600 text-white hover:bg-blue-700 p-2"
                      size="sm"
                      title="Use in this section"
                    >
                      Link
                    </Button>
                    <Button
