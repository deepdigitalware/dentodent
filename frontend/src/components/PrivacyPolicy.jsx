...
const PrivacyPolicy = () => {
  ...
  const dynamicText = content?.privacyPolicy?.content;
  const dynamicTitle = content?.privacyPolicy?.title || 'Privacy Policy';
  const effective = content?.privacyPolicy?.effectiveDate || 'January 1, 2024';
  const updated = content?.privacyPolicy?.lastUpdated || 'January 15, 2024';
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        >
          ...
          <h1 className="text-4xl lg:text-5xl font-display font-bold mb-6">
            {dynamicTitle.split(' ').slice(0, 1).join(' ')} <span className="gradient-text">{dynamicTitle.split(' ').slice(1).join(' ') || 'Policy'}</span>
          </h1>
